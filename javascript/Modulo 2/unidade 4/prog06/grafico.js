function Grafico(parametros) {
    ...
    let margem = { esquerda: 70, direita: 20, superior: 40, inferior: 100 };
    ...
    this.atualizar = (novosDados) => {
        ...
        let self = this;
        let retangulos = this.plotagem.selectAll(".barras")
            .data(novosDados);
        retangulos.enter().append("rect")
            .classed("barras", true)
        ...
        this.plotagem.selectAll(".barras").transition().duration(this.duration)
        ...
        let rotulos = this.plotagem.selectAll(".rotulo")
            .data(novosDados);
        rotulos.enter().append("text")
            .classed("rotulo", true)
            .text(d => d.valor);
        rotulos.exit().remove();
        ...
        this.plotagem.selectAll(".rotulo").transition().duration(this.duration)
            .text(d => d.valor)
            .attr("x", (d, i) => this.fnX(d.chave))
            .attr("dx", d => this.fnX.bandwidth() * 0.5)
            .attr("y", (d) => this.fnY(d.valor))
            .attr("dy", -5);

        this.duration = 500
    }
    this.atualizar(parametros.dados);
}